...
dbSendQuery(con, "CREATE TEMP TABLE bab_stations AS 
            ...
            WHERE LOWER(street) ~ '(a\\d+|a \\d+|bab|autohof|rasthof)'")
...
reg.fe <- plm(price ~ bab_station + rerun + fuel_type + route_length + within + brand, 
              data=refueling.events, 
              index=c("c_id"), 
              effects="individual",
              model="within")

# Save Information
sink(file="reg1_fixedeffects.txt", append=FALSE)
summary(lm1)
